...
class Adapter(BaseAdapter):
    ...
    async def _startup(self):
        ...
        bot = Bot(self, "claweixin_bot")
        self.bot_connect(bot)
        log("DEBUG", f"Bot {bot.self_id} connected.")
        self.tasks.append(asyncio.create_task(self._poll_updates(bot)))
    ...
    async def _poll_updates(self, bot: Bot):
            api_root = getattr(self.claweixin_config, "claweixin_api_root", "https://ilinkai.weixin.qq.com")
            token = getattr(self.claweixin_config, "claweixin_token", "")
            get_updates_buf = ""
    ...
    async def _call_api(self, bot: Bot, api: str, **data: Any) -> Any:  # type: ignore
        ...
        if api == "send_message":
            ...
            elif hasattr(message, "type"): # MessageSegment
